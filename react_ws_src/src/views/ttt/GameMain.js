...
import nextMove from "../../helpers/find_best_move"
import checkWin from "../../helpers/check_wins"
...
export default class SetName extends Component {
	...
	turn_comp() {
		...
		const c = nextMove(cell_vals, app.settings.level)
		cell_vals[c] = "o"
	...
	check_turn() {
		const { cell_vals } = this.state
		const winSet = checkWin(cell_vals)
		let fin = true
		...
		if (winSet) {
			...
			this.setState({
				game_stat: (cell_vals[winSet[0]] == "x" ? "You" : "Opponent") + " win",
				game_play: false
			})
